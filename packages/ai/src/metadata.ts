...
export const aiMetadata: ZeusWebAiMetadata = {
  ...
  advancedComponents: [
    ...
    },
    {
      ...
      category: 'advanced',
      summary:
        'Use @zeus-web/data-grid to build headless data grids with row virtualization, column model, row model, selection and sorting.',
      whenToUse: [
        '需要展示结构化表格数据时使用。',
        '需要轻量 DataGrid 而不是 AG Grid 级完整表格系统时使用。',
        '需要跨 Native Web Component、React、Vue 复用同一套 DataGrid 行为协议时使用。',
        '需要行虚拟滚动、单列排序、单选或多选时使用。',
      ],
      doNotUseFor: [
        '不要把它当作服务端数据源。',
        '不要在组件内部写真实请求逻辑。',
        ...
        '不要在 Phase 6 中依赖过滤器、树表、分组、列虚拟或单元格编辑器。',
      ],
      tags: [
        'data-grid',
        'table',
      ...
      slots: {
        'zw-data-grid': ['empty', 'header-{columnId}', 'cell-{columnId}'],
      },
      events: {
        'zw-data-grid': [
          'range-change',
